"""Génération à la volée de séquences : disque rebondissant + barre occlusive fixe.
...
def simulate_trajectory(cfg: Config, rng: np.random.Generator,
                        ensure_crossing: bool) -> np.ndarray:
    ...
    if ensure_crossing:
        ...
        t_c = max(1, min(t_c, int((bar_center - lo - 1) / vx)))
...
def render_frames(cfg: Config, positions: np.ndarray,
    ...
    if with_bar:
        x0 = int(round(cfg.bar_x))
        x1 = int(round(cfg.bar_x + cfg.bar_width))
        frames[:, :, x0:x1] = cfg.bar_color
    return frames[:, None]


def occlusion_flags(cfg: Config, positions: np.ndarray) -> np.ndarray:
    """(T,) bool : True quand le disque est totalement derrière la barre."""
    x = positions[:, 0]
    return (x >= cfg.bar_x + cfg.disk_radius) & \
           (x <= cfg.bar_x + cfg.bar_width - cfg.disk_radius)
...
def make_sequence(cfg: Config, rng: np.random.Generator) -> dict:
    ...
    positions = simulate_trajectory(cfg, rng, ensure_crossing)
    return {
        "frames": torch.from_numpy(render_frames(cfg, positions)),
        "positions": torch.from_numpy(positions),
        "occluded": torch.from_numpy(occlusion_flags(cfg, positions)),
    }
